# -*- coding: utf-8 -*-
"""Cinejoy adaptive provider (api.shegu.st / TMDB IDs)."""

from __future__ import annotations

from providerModules.a4kEmbeds import cinejoy_api
...
def _build_sources(title_label, streams, extra=None):
    ...
    for stream in streams:
        ...
        if not url:
            continue
        server = stream.get("server") or "Cinejoy"
        entry = {
            "release_title": f"{title_label} [Cinejoy {server}]",
            "quality": "1080p",
            "url": url,
            "referer": cinejoy_api._REFERER,
            "info": {"CINEJOY", server.upper()},
            "cinejoy": True,
            "cinejoy_server": server,
        }
        entry.update(extra)
...
class sources:
    def episode(self, simple_info, all_info, **kwargs):
        ...
        if not tmdb_id:
            tools.log("cinejoy: missing tmdb_id", "info")
            return []
        ...
        try:
            streams = cinejoy_api.resolve_episode_all(
                tmdb_id,
                season,
                episode,
                title=title,
            ...
            tools.log(f"cinejoy: resolve failed ({exc})", "info")
            return []
    ...
    def movie(self, title, year, imdb_id=None, **kwargs):
        ...
        tmdb_id = _tmdb_id(simple_info, kwargs.get("info"))
        if not tmdb_id:
            tools.log("cinejoy: missing tmdb_id for movie", "info")
            return []

        movie_title = title or simple_info.get("title") or "Unknown"
        movie_year = year or simple_info.get("year")
        try:
            streams = cinejoy_api.resolve_movie_all(
                tmdb_id,
                title=movie_title,
                year=movie_year,
            )
        except Exception as exc:
            tools.log(f"cinejoy: movie resolve failed ({exc})", "info")
            return []
...
def get_listitem(source):
    ...
    if not play_url:
        raise Exception("cinejoy: empty stream url")

    play_url = cinejoy_api.prepare_playback_url(
        play_url,
        server=source.get("cinejoy_server"),
    )
    headers = cinejoy_api.playback_headers()
